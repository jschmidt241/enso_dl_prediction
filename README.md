...
This work is adapted from Environmental ML coursework, updated and improved with 2026 data, more sophisticated tuning, and better hardware. Most recent outlooks point to an upcoming very strong El Niño. Can a convolutional neural net that's only seen two very strong events (1997 and 2015, using data from 1993-2026), trained on monthly sea surface temperature anomaly (SSTa) data, and targeting sea surface height (SSH) anomaly values get close to much more sophisticated methods?
...
<br><br>
So far, none of these models have predicted a very strong El Niño event (which I'm loosely defining as an SSH Index > 2) in the coming months. These preliminary results are still promising especially considering the limited input SSTa data, and future work could likely close that gap.
...
## Future Work
Next steps:
* Train a phase classification task predicting very strong events with lead times > 1 month
* Try targeting NOAA's [Relative Oceanic Niño Index (RONI)]([url](https://www.cpc.ncep.noaa.gov/products/analysis_monitoring/enso/roni/)) instead of SSH anomaly.
* Continued work adjusting the CNN model architecture
* Try Huber loss instead of MSE to filter out some of the aggressive noise present in SST data.
* Find datasets with SSTa that extend before 1993
